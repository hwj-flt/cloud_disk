...
分享的文件列表，显示文件名、类型（私密、外链）、状态（是否过期）、分享人、分享时间；清空无效分享；可对链接进行复制、删除、下载文件、转存文件（选择目标文件夹）

## 4.2我分享的

显示所有我分享过的文件/文件夹列表，用户选择对应项后，可以点击修改密码和有效期，弹窗输入框，用户可以输入密码和修改有效期（不能改为今天之前），点击确定后保存到数据库；用户还可以点击取消分享，点击后弹窗确认，若取消修改，则修改数据库，并提示结果；用户可以点击复制分享链接，点击后将链接复制到剪切板。

![image-20200917162234899](.\img\image-20200917162234899.png)

链接列表，显示文件名、链接状态（是否过期）、分享类型（私密、外链）、分享时间，清空无效分享；修改分享（显示分享链接，修改分享对象或添加对象、有效期、密码）、取消分享（可以批量取消）
...
使用管理用户模块的查询用户功能显示普通用户

为了批量选择，需要加一个群组查询，即显示该群组下的所有用户，所以是按群组显示
...
ALTER TABLE DIRECTORY ADD DIRECT_DELETE_TIME   TIMESTAMP COMMENT '删除文件的时间'
...
3. 永久删除文件或文件夹
   ...
   ​			文件夹删除（找到文件夹表对应文件夹，编译文件夹下的文件夹和文件，删除文件时，文件表中的引用减一，修改剩余空间大小）
...
## 7.1上传文件（同我的文档模块，需有权限）

## 7.2文件处理

### 7.2.1下载（同我的文档模块，需有权限）
...
### 7.2.3重命名（同我的文档模块，需有权限）

### 7.2.4复制到（同我的文档模块，需有权限）

### 7.2.5移动到（同我的文档模块，需有权限）

### 7.2.6分享（同我的文档模块，需有权限）

## 7.3新建文件夹（同我的文档模块，需有权限）
